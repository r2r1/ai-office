...
| `office/costs.py` | Учёт токенов и стоимости по агентам (прайс apinet); `record()` зовётся из `core/llm.py` |
| `office/workspace.py` | Рабочая папка `workspace/` — агенты пишут реальный код (`write_file`/`read_file`/`list_files`), защита от выхода за пределы каталога |
| `office/state.py`, `memory.py`, `brief.py` | Состояние, память, бриф клиента |

### Мультиарендность (`src/saas/`, Phase 0–1)
Сервис многопользовательский. `saas/context.py` хранит `tenant_id` в ContextVar
(ставится middleware из сессии на каждый HTTP-запрос; офис-задача ставит его на
свой цикл — дочерние `asyncio`-задачи наследуют через copy_context). Все данные
офиса теперь **per-tenant и stateless**: модули `office/*` читают/пишут
`data/tenants/<tid>/<name>.json` через `context.read_json/write_json` (нет общих
глобалей → нет утечки между клиентами). `saas/db.py` (sqlite) — таблицы
`users`/`workspaces`; `saas/auth.py` — сессии (PyJWT) + GitHub OAuth + dev-вход;
`saas/store.py` — пользователи/тенанты. `office/loop.run()` — **менеджер**: опрашивает
workspaces и запускает свой офис на каждого тенанта с готовым брифом. Лендинги
публичны по `/site/{tenant}/{slug}`, лиды — `/api/lead/{tenant}/{slug}`.
Каждый тенант использует **СВОЙ ключ к LLM** (`office/llm_settings.py`: base_url +
api_key, ключ шифруется через `saas/crypto.py` Fernet от `APP_SECRET`); `core/llm.py`
берёт креды текущего тенанта, иначе fallback на `LLM_*` из .env. Креды интеграций
(`connections.py`) тоже шифруются at-rest (агентам — расшифровка, в UI — маска).
Следующее (Phase 3+): очередь задач, GitHub-пуш/деплой, биллинг.
...
- `telegram.py` — эталонная интеграция с кредами (Bot API: `get_me` / `send_message` / `send_photo`).
- `github.py` — создание репозитория (`create_repo`) и пуш рабочей папки тенанта
  (`push`) через Contents API. Токен сохраняется автоматически при «Войти через
  GitHub» (scope `repo`) или вводится вручную (PAT).
- `website.py` — интеграция БЕЗ кредов: публикует реальный лендинг (`publish_landing`),
  хостит его сам бэкенд (`GET /site/{slug}`), форма собирает живых лидов
  (`POST /api/lead/{slug}` → `office/leads.py`). Артефакт + реальные данные «из коробки».
  Интеграции с пустым `cred_fields` считаются всегда подключёнными (`is_connected`).
...
- `index.html` — разметка, CSS, навигация по вкладкам (Офис / Команда / Результаты /
  Лиды / Код / Доступы / События / Этапы / Чаты). Вкладка «Код» — файлы из рабочей
  папки, которые написали разработчики. Вкладка «Лиды» — опубликованные
  лендинги (ссылки) и реальные заявки с их форм. Вкладка «Доступы» показывает каталог
...
- `GET /site/{slug}` — опубликованный лендинг; `POST /api/lead/{slug}` — приём заявки.
- `GET /api/sites`, `/api/leads` — лендинги и собранные лиды (вкладка «Лиды»).
- `GET /api/costs` — расход токенов и стоимость (индикатор 💸 в топбаре, разбивка по агентам).
- `GET /api/files`, `/api/file?path=` — код проекта из рабочей папки (вкладка «Код»).
...
- **Единая ответственность**: `MAX_PER_ROLE` (=1) — на каждую рабочую роль ОДИН агент.
  Если исполнитель занят — директор ждёт, а не клонирует роль (`orchestrator.py`).
- **Приоритет указаний пользователя**: ответы пользователя (`memory`) подмешиваются в
  контекст директора и ГЛАВНЕЕ стратегии/ТЗ. Нельзя ставить задачи, противоречащие им
  (пример: клиент сказал «не BotsBox» → задачи про BotsBox запрещены).
- **Реальный код, не описания**: разработчик пишет код в рабочую папку через `write_file`,
  проверяет через `verify_code` (компиляция), и пушит в GitHub (`use_integration github`)
  ТОЛЬКО после одобрения пользователя (спрашивает через `ask_user`). См. `office/workspace.py`. Платные конструкторы (Tilda/BotsBox)
  без одобрения клиента не подключаются: сайт — через `website`, бот — собственный код.
...
- **ТЗ архитектора** — техническая основа, но указания пользователя её переопределяют.
