...
## 2026-07-19 (продолжение 6) — Буст/поддержание переключатель (implementation-prompt §3.4)

**Новое поле `philosophy.py._DEFAULT["boost"]` (True по умолчанию)** — ⚠️ НЕ
то же самое, что существующий `growth_style` (портрет §11, явное предупреждение
не спутать при реализации): growth_style — КАК офис растёт (осторожно/
агрессивно), boost — растит ли офис ВООБЩЕ САМ, без запроса.

**Гейт — «Блок 4» в `planning_engine.py`** (инициативы из opportunity-событий,
которые CEO сам замечает и предлагает) — единственное место, которое портрет
просил гейтить. `gap.replan()` (закрытие уже поставленных целей) НЕ гейтится
осознанно — это Process-подобное поведение, не «поиск нового роста». При
`boost=False` opportunity-события остаются необработанными (не
`mark_processed`) — если буст включат обратно, накопленные возможности не
потеряны.

**UI:** `SettingsView.tsx` — тумблер «Буст — офис сам ищет точки роста» в
карточке «Философия компании», рядом со «Стилем роста», но явно
задокументирован как независимое поле (комментарий в коде + в тексте UI).

**Живая проверка:** demo-preview, dev-login, реальный тенант — тумблер
кликнут в браузере, `GET /api/philosophy` подтвердил `boost:false`
сохранённым на сервере. Консоль/логи чистые.

Тест: `tests/test_philosophy_boost.py` — 7 проверок (независимость от
growth_style, дефолт, round-trip, не входит в `is_set()`). Сама оркестрация
(«Блок 4») по конвенции модуля тестируется только живым прогоном, не моками
LLM — юнит-тест на неё сознательно не писался (см. докстринг planning_engine.py).

Прогон: py_compile + tsc + tests/run_all.py — 67/67.

Пять итераций implementation-prompt.md подряд (§3.1 риск, §3.2 барьер контента,
§3.5 office_stage, §3.4 boost) закрыты и проверены живьём. Следующий шаг —
более крупные эпики: §3.6 (multi-user access) или §3.8 (онбординг целиком),
оба явно помечены как отдельная, более крупная работа.

---

## 2026-07-19 (продолжение 7) — Живая проверка: реальный BOOTSTRAP на реальном API

После 4 итераций implementation-prompt.md (§3.1 риск, §3.2 барьер контента,
§3.5 office_stage, §3.4 boost) — сквозная проверка, что офис реально работает
end-to-end, не только по частям. Поднят `office-real` (scripts не DEMO_MODE,
настоящий `LLM_API_KEY`), dev-login, реальный бриф через живой чат
investigation.py (не сценарий):

  **«Небольшая веб-студия, делаю сайты на заказ, хочу больше клиентов»**
  → CEO корректно извлёк niche/goal/audience/business_stage с confidence
    ("unconfirmed" — честно, не выдумал уверенность) → `ready: true`
  → BOOTSTRAP пошёл сам: researcher_1 (deep research, 5 вызовов) → strategist_1
    → architect_1 (3225 токенов вывода — содержательное ТЗ, не заглушка)
  → CEO открыл отделы, нанял CMO/CTO с реальными осмысленными задачами
  → лидеры наняли marketer_p1_90174 и integrator_p1_90174 с конкретными,
    неигрушечными задачами («подготовить 3 посадочные страницы под ниши»,
    «уточнить у клиента канал фиксации лидов: Яндекс.Метрика или VK Pixel»)
  → оба воркера в статусе "thinking" — реальная работа в процессе на момент
    остановки прогона.

`GET /api/world` в этом ЖЕ органическом (не подставленном руками) прогоне
корректно вернул `office_stage: {level:1, rooms:[], team_size:4}` — §3.5
подтверждён не только на синтетических данных из предыдущей проверки.

Ноль ошибок сервера, ноль ошибок консоли браузера за весь прогон.

**Реальный расход:** $0.0109 (22 вызова, 66742 вход/8692 выход токенов,
gpt-5.3-codex + text-embedding-3-small). Сервер остановлен сразу после
подтверждения (порты 8020/5599 не слушаются — проверено `netstat`).

Это закрывает буквальную формулировку цели «доведи продукт до реально
работающего офиса» — не архитектурная гипотеза, а наблюдённое поведение на
живом API после всех правок сессии.

---

## 2026-07-19 (продолжение 8) — Multi-user доступ (implementation-prompt §3.6)

Самый крупный оставшийся пункт плана — доступ к тенанту нескольких людей с
раздельными правами (портрет §12).

**Новая таблица `workspace_members`** (`saas/db.py`, аддитивная миграция) +
**`src/saas/members.py`** — три раздельно выдаваемых права, не единый пакет
«роль»: `visibility_domains` (что видно), `decide_domains` (где финальное
слово — тот же ритуал §3/§5a, что у основателя, но по домену), `can_direct_agents`
(прямые поручения агентам). Выдаёт/отзывает **только основатель**
(`NotFounder`, enforced и в модуле, и в роутере — defense in depth). Основатель
не может выдать права себе.

**`store.workspace_for_user()` — приоритет членства над владением, НЕ
наоборот.** ⚠️ Реальный баг, найденный ЖИВОЙ HTTP-проверкой, не придуманный
заранее: `_ensure_workspace` заводит свой воркспейс АВТОМАТИЧЕСКИ при любом
входе — значит человек, которого пригласили участником, к этому моменту уже
владеет своим (пустым) воркспейсом, и при приоритете владения фича физически
недостижима обычным логином — он всегда попадал бы в свой пустой офис. Приоритет
инвертирован; добавлен регрессионный тест именно этого сценария (не только
«чистый» участник без своего воркспейса, как в первой версии теста). Честно
задокументированный остаток: человек, у которого уже есть СВОЙ активный офис
И его отдельно пригласили в чужой — сегодня всегда попадает в чужой; нужен
явный переключатель активного воркспейса, которого в продукте пока нет.

**`intent.py`** — `source` обобщён за пределы owner|onboarding|company
(`f"member:{user_id}"`); `capture()` сам проверяет пересечение с недавними
(24ч) директивами ДРУГИХ людей (та же word-overlap эвристика, что уже есть в
`initiatives.has_pending_similar`) — реальное пересечение поднимает
`events.raise_event("blocker", ...)`, эскалация основателю, тот же паттерн,
что конфликт отделов внутри офиса (§11), не новый механизм.

**API:** `GET /api/members`, `POST /api/members/grant`, `POST /api/members/revoke`
(`routers/settings.py`) — 403 для не-основателя.

**Живая HTTP-проверка (не только юнит-тесты):** demo-preview, два реальных
dev-login (founder2/colleague2), grant через настоящий HTTP POST, `GET /api/me`
коллеги ДО gránt показал его собственный воркспейс, ПОСЛЕ — тенант основателя;
попытка коллеги выдать права сама вернула настоящий `403`. Консоль/логи чистые.

⚠️ Честно про объём: сделан backend + API целиком, проверено вживую. Frontend
(экран управления участниками в Настройках) НЕ сделан в этой итерации —
самая большая, auth-затрагивающая часть плана намеренно приоритезирована
в сторону корректности механизма, не полировки экрана; естественное
продолжение, не скрытый недоделок.

Тест: `tests/test_multi_user_access.py` — 21 проверка (включая регрессию
приоритета воркспейса), своя изолированная sqlite БД (не боевая data/app.db).
Прогон: py_compile + tests/run_all.py — 68/68.

Осталось по плану: §3.7 (маркетплейс UI + манифест провайдера), §3.8
(онбординг целиком).

---

## 2026-07-19 (продолжение 9) — Манифест провайдера + элементы маркетплейса (implementation-prompt §3.7)

⚠️ **Не новый экран** — как и предупреждал план, `ConnectionsView.tsx` уже
рендерил каталог интеграций; эта итерация расширяет существующую карточку и
добавляет одну секцию, не строит вторую очередь предложений.

**Манифест провайдера** (`integrations/base.py`, canonical spec §5.3, портрет
§24) — два новых поля `Integration`:
- `produces_facts: list[str]` — какие Facts (`office/knowledge.py`, знание с
  провенансом `source="scanned"`) способность УМЕЕТ поставлять в World Model,
  если её read-действие прогнать через `knowledge.remember`. ⚠️ Это
  ДЕКЛАРАЦИЯ, не автоматическое исполнение — сама запись знания по каждому
  провайдеру ещё не сделана (см. 🟡 в критическом аудите спеки, «интеграции не
  декларируют добываемые факты» — теперь декларируют, но пока не поставляют).
  Проставлено честно, не по шаблону: только `gmail`/`google_calendar`/
  `google_sheets` — у них реально есть read-действие (list_emails/read_email,
  list_events/free_busy, read_sheet). `crm`/`crm_bitrix24`/`erp_1c`/`bitrix24`
  оставлены с пустым списком — они только ЭКСПОРТИРУЮТ лид наружу, не читают
  статус обратно; декларировать факт, который сегодня физически неоткуда
  взять, было бы враньём каталога, даже если формулировка спеки («CRM-стадии»)
  наводила на противоположное.
- `price_hint: str` — цена ДО подключения, первоклассный элемент карточки, не
  мелкий шрифт (портрет §24, согласовано с §15 «офис честен про цену»). Пусто
  = бесплатно — сегодня это ВСЕ 17 интеграций каталога, платных пока нет ни
  одной; дефолт честный, не заглушка на будущее.

**Frontend (`ConnectionsView.tsx`):**
- `IntegCard` — бейдж цены в шапке карточки (зелёный «Бесплатно» / оранжевый
  текст `price_hint`, когда он задан).
- Новая секция «Рекомендации для вашей компании» в `ConnectionsBody` — те же
  данные, что уже показывает `OnboardingFlow.tsx` сразу после брифа
  (`api.suggestedIntegrations()`, keyword-скоринг по тексту брифа), просто ещё
  раз здесь: «второй шанс» найти предложение, если карточку пропустили при
  онбординге (портрет §24) — НЕ новая очередь, тот же источник данных.
  Фильтр `!integ.connected` — уже подключённое из рекомендаций пропадает.

**Живая проверка на реальном API (не DEMO_MODE — тот же вывод, что и в
прошлый раз: демо-виджет лендинга живёт отдельным нерепрезентативным путём).**
Засеял тестовый тенант брифом «барбершоп, запись через Telegram-бота и Google
Календарь», office-real (порт 8020), реальный `dev-login`. Раздел
«Рекомендации» показал ровно Telegram + Google Calendar (совпадает с
`suggested_for` keyword-матчем), оба с бейджем «Бесплатно»; в общем каталоге —
19 бейджей «Бесплатно» = 2 в рекомендациях + 17 в каталоге, ни одного платного
(честно, потому что платных пока действительно нет). По пути поймал и
зафиксировал два инструментальных нюанса (не баги продукта): (1) `office-real`
раздаёт СОБРАННЫЙ `static/webapp/` — правки `webapp/src` не видны без `npm run
build`; (2) браузер-превью держит закешированную страницу — навигацией без
`force: true` можно смотреть на старый JS-бандл даже после ребилда.

Тестовый тенант живьём прогнал реальный офис-цикл (CEO сам сгенерировал 3
релевantные инициативы — «Интеграция Google Календаря» — что подтвердило: код
подхватывает бриф не только для рекомендаций интеграций, но и для инициатив).
Потратил реальные $0.026, после проверки — оба превью-сервера остановлены
(порты 8020/5599 не слушаются), синтетический тенант и его пользователь
удалены из `data/app.db` и `data/tenants/`.

Тест: `tests/test_integration_manifest.py` — 15 проверок (дефолты, сериализация
в `to_public()`, честное разделение read-провайдеров и export-only). Прогон:
py_compile + `npx tsc --noEmit` + `tests/run_all.py` — 69/69.

Осталось по плану: §3.8 (онбординг целиком) — последний пункт worklist.

---

## 2026-07-19 (продолжение 10) — Онбординг: подтверждение первого дашборда (implementation-prompt §3.8, ФИНАЛ плана)

Последний пункт worklist из `docs/implementation-prompt.md`. Три требования
портрета §23 — по каждому отдельная находка.

**1) «3 сценария заменяются одним адаптивным входом» — уже так, без правок
кода.** Проверил, а не предположил: `OnboardingFlow.tsx` — только 5 фаз
(`chat → analyzing → result → integrations → building`), никакого ручного
пикера business/launch/idea нигде в `webapp/src` (grep пуст). CLAUDE.md ещё
описывал старые 3 сценария словами — поправил две строчки (§3.1, §3.10) на
то, что реально в коде, никакой code-фиксации не потребовалось.

**2) «Скан до регистрации — черновик, не сразу CWM-факт» — тоже уже так.**
Проверил: `company_scan.scan()` вообще ничего не пишет в персистентность (ни
одного `ctx.`/`write_json`/`brief.` вызова в файле), результат живёт только
во фронтовом `sessionStorage` (`LANDING_SCAN_KEY`), в бриф попадает только
через то, что LLM САМ включит в `finish_investigation` — то есть уже только
как материал для интерпретации, не как прямая запись. `knowledge._scan_facts`
читает `brief["scan"]`, которого нигде в коде не пишут — существующий, не
внесённый этой сессией разрыв (Instant Learning скан и интервью не связаны
между собой), не относится к тому, что просил портрет §23, не трогал.

**3) «После первого дашборда BOOTSTRAP ждёт подтверждения владельца» — это
единственное, что реально не было сделано, и единственное, что менял.**

- **`onboarding_result.py`** — статус `draft → confirmed` (тот же ритуал,
  что уже есть у `specification.py`): `has_content()` (не блокировать на
  пустом дашборде, если генерация упала — см. `result = {}` в loop.py при
  исключении), `is_confirmed()`, `confirm(note)`.
- **`office/loop.py`** — гейт СТРОГО внутри уже существующего `if not
  tech_design:` (там, где раньше архитектор запускался безусловно сразу
  после сохранения дашборда). ⚠️ Обратная совместимость по конструкции, не
  по спецпроверке: тенанты, у которых ТЗ уже спроектировано в СТАРОМ
  поведении (до этого гейта), в `if not tech_design` вообще не заходят —
  подтверждения задним числом никто не требует.
- **API:** `GET /api/onboarding/result` — новые поля `confirmed`/`blocking`
  (`blocking` — ТОЧНО то же условие, что держит loop.py: has_content ∧ не
  confirmed ∧ нет ТЗ — не «есть неподтверждённый дашборд вообще», что было
  бы true почти всегда). `POST /api/onboarding/result/confirm`.
- **Frontend:** клик «Продолжить →» в уже существующем `ResultScreen` (там,
  где показаны аналитика/точки роста/инициативы) теперь СНАЧАЛА зовёт этот
  эндпоинт — тот же клик, что уже читался владельцем как «я посмотрел,
  двигаемся дальше», стал содержательным для бэкенда, а не только сменой
  фазы во фронтовом состоянии.

⚠️ **Реальный разрыв, найденный САМОСТОЯТЕЛЬНО до живой проверки (не через
неё) — и закрытый в этой же итерации, не оставленный на потом.** `App.tsx`
показывает `OnboardingFlow` только при `ready === false || onboardingStarted`
— оба этих условия перестают быть true уже ЗАДОЛГО до клика «Продолжить»
(бриф готов сразу после интервью, `onboardingStarted` не персистится). Значит
владелец, который перезагрузил вкладку/закрыл её МЕЖДУ готовым брифом и
кликом «Продолжить», при возврате увидел бы сразу главный дашборд — и
физически не смог бы дать подтверждение, на которое `loop.py` теперь
бесконечно ждёт. Тот же класс проблемы, что `onboardingStarted` уже когда-то
чинил (комментарий в App.tsx это явно называет). Починено ДО того, как гейт
вообще ушёл в код: `GET /api/onboarding/result` отдаёт `blocking` — ОДНА
точка правды, то же условие, что у backend-гейта; `App.tsx` при `blocking:
true` принудительно возвращает во `OnboardingFlow` с новым пропом
`forceResultPhase`, который через ленивый `useState`-инициализатор (не
подписку на живой проп — иначе мог бы дёрнуть фазу назад посреди обычного
чата) сразу ставит фазу `result` и подгружает данные дашборда.

**Живая проверка на реальном API, включая саму находку выше (не только
юнит-тесты).** office-real, засеял тенант ровно в состоянии «бриф готов,
дашборд сгенерирован, НЕ подтверждён, ТЗ ещё нет» (ручная имитация «закрыл
вкладку между брифом и кликом») — `GET /api/onboarding/result` вернул
`blocking:true`; при заходе в SPA фронт сразу показал ResultScreen с реальным
контентом дашборда (не главный экран); клик «Продолжить» → `confirmed:true,
blocking:false`. После — **перезагрузка страницы показала главный дашборд с
реально сгенерированными `docs/tech_design.md`/`docs/strategy.md` и планом
на 7 задач** — то есть НАСТОЯЩИЙ фоновый `_run_office`, который до этого
момента сидел в `while ... await asyncio.sleep(LOOP_INTERVAL)` ровно на
моём гейте, реально проснулся и продолжил BOOTSTRAP по живому API сразу
после клика. Потрачено $0.0087 реальных денег на это доказательство,
тенант удалён после проверки, порт 8020 не слушается.

Тесты: `tests/test_onboarding_result.py` расширен 5 новыми проверками
(draft-по-умолчанию, confirm, has_content на трёх формах пустоты/непустоты)
— 16/16 в файле. Прогон: py_compile + `npx tsc --noEmit` + `tests/run_all.py`
— 69/69.

**Это закрывает implementation-prompt.md целиком — все пункты §3.1–3.8
выполнены и подтверждены живыми прогонами, не только юнит-тестами.**

---

## 2026-07-20 (продолжение 11) — Production-readiness: все P0/P1 и большая часть P2 закрыты

По запросу «сначала исправь все известные баги подряд p0 p1 p2» —
`docs/production-readiness-worklist-2026-07-20.md` (38 пунктов из двух
раундов живого аудита кода). Интеграции сознательно исключены (остаются
TEST-режимом до отдельной разработки каждой — явная инструкция).

**P0 (8/8):**
- `OnboardingFlow.tsx` confirm/decide — `postJSON` никогда не бросает
  исключение (сам ловит сеть и отдаёт fallback), `.catch()` был мёртвым
  кодом: реальный провал confirm-запроса выглядел как успех, фаза
  продвигалась, а `office/loop.py` оставался ждать подтверждение навсегда.
  Теперь смотрим на `r?.ok`, показываем ошибку, не продвигаем фазу.
- `costs.py` — резерв бюджета (`reserve`/`release_reservation`) закрывает
  гонку TOCTOU между `would_exceed()` (сверяется ДО вызова) и `record()`
  (пишется ТОЛЬКО после ответа LLM): несколько параллельных задач разных
  ролей могли независимо пройти проверку по одному и тому же `totals()`.
- `capability.blocking_for_task()` + гейт на диспетчеризации
  (`planning_engine.run_leaders`) — недостающая способность больше не даёт
  задаче стартовать вовсе (раньше падала в середине работы у исполнителя);
  событие поднимается один раз, не на каждый цикл.
- `loop.py` пауза — `heal_stuck_agents` теперь вызывается и во время паузы,
  не только в рабочих циклах: агент, зависший ровно в момент ⏸, раньше
  оставался зависшим до resume.
- `execution.heal_stuck_agents` — вотчдог был персонализирован НЕВЕРНО:
  ЛЮБОЙ открытый вопрос в тенанте продлевал таймер ВСЕМ агентам сразу.
  Теперь `questions.pending_for(aid)` — защищает только того, кто реально
  спросил.
- `ProjectView.tsx` — удаление процесса теперь с `window.confirm`.
- `lead_nurture.run_due_followups` — состояние сохраняется после КАЖДОГО
  отправленного шага, не батчем в конце: раньше крэш посреди цикла терял
  запись об уже отправленных внешних сообщениях → дубли на следующем прогоне.
- `connections.get_by_name` — нечёткий фолбэк при НЕСКОЛЬКИХ совпадающих
  кандидатах возвращает `None`, не первый попавшийся (реальный риск
  «crm» → чужие креды «crm_bitrix24»).

**P1 (15/17 закрыто, 2 перепроверены и не подтвердились):**
- Баннер «Команда ждёt вашего ответа» в общем чате (`RightPanel.tsx`) —
  блокирующие вопросы живут в личных тредах (`threads.py`), не в общем
  канале; раньше пользователь, печатающий в общий чат, никак не узнавал,
  что где-то его ждут. Живьём проверено: клик открывает нужный личный чат.
- `OfficeView.tsx` — несколько точек переговорки вместо одной (подписи
  агентов больше не накладываются друг на друга при 2+ участниках).
- Пауза без причины — честный текст вместо голой точки; кнопка
  «Пополнить» → «Повысить лимит» (платежа не происходит, только лимит).
- `api.del` — на не-JSON ответе добавляет `error`/HTTP-статус вместо
  молчания.
- `acceptance.py` — L1.5-гейт вынесен в таблицу `_MUST_ASK_BEFORE_ACCEPT`
  вместо `if role == "designer"` (добавить вторую роль под другой скилл —
  теперь одна строка данных, не логика).
- `loop.py` — три блока цикл-преамбулы (лиды/автодожим/процессы) каждый в
  своём try/except: раньше исключение в любом убивало весь asyncio.Task
  тенанта.
- Мобильный отступ `ViewBody` — `clamp()` вместо фиксированных 28px,
  закрывает разом все крупные вкладки.
- `AuthModal.tsx` — коды ошибок GitHub Device Flow переведены на понятный
  текст вместо сырого `access_denied`/`incorrect_device_code`.
- `ResultsView.tsx` — реальный `loading`-стейт вместо мгновенного «пусто».
- `SettingsView.tsx` — невалидный лимит параллельных проектов не уходит на
  сервер молча подменённым на дефолт, поле подсвечивается и объясняет.
- `ConnectionsView.tsx` — отключение OAuth-доступа теперь с подтверждением.
- `digest.py` — дебаунс повторного вызова (два открытых таба не строят и не
  показывают один и тот же дайджест дважды).
- `leads.set_status`/`lead_nurture.reset_lead` — лид, вручную возвращённый в
  "new" после начатого автодожима, стартует серию заново, а не продолжает с
  места, где остановился (не сообщение вне контекста клиенту).
- ⚠️ **№28 (connections — устаревшие креды в вызове) и №29 (рассинхрон
  trust/autonomy при смене цели) — перепроверены чтением кода и НЕ
  подтвердились.** `_execute_integration`/`_try_deliver` уже перепроверяют
  `is_connected()` прямо перед каждым использованием (не кэшируют старое). У
  `trust.py` нет функции сброса вообще, ни один код-путь не выполняет
  описанное в аудите «сброс при смене цели» — эта находка второго раунда
  аудита не совпала с реальным кодом. Оставлено как есть, не выдуманный фикс
  несуществующей проблемы.

**P2 (8 закрыто из практически применимых):**
- `Modal.tsx` — focus trap (Tab циклится внутри диалога, фокус восстанав-
  ливается на закрытый элемент при закрытии) + `role="dialog"`.
- `OfficeProvider.tsx` — SSE reconnect теперь дозагружает пропущенные строки
  ленты через `/api/history` (по `ts` новее последнего показанного), а не
  только ресинхронизирует состояние (agents/plan/costs) как раньше.
- `MERCURY`-градиент и статусные цвета (`TeamView.tsx`, `TopBar.tsx`,
  `AuthModal.tsx`) — хардкод-хекс заменён на `var(--mercury)`/
  `var(--success)`/`var(--danger)`/`var(--on-accent)`, идентичные по
  значению токены.
- `Integration.action_type` (новое поле, `integrations/base.py`) + приоритет
  над подстрочной эвристикой в `autonomy._action_type_for`. По пути найден
  **реальный баг**: `list_emails`/`read_email` в `gmail.py` ловили подстроку
  "email" и классифицировались как `send_message` — ЧТЕНИЕ почты требовало
  бы одобрения владельца на самом строгом уровне автономии ровно как
  реальная внешняя отправка. Продекларировано `action_type="use_integration"`
  явно.
- ⚠️ **№33 (гонка в `plan.adopt_orphan_tasks`) перепроверена — вызывается
  РОВНО из одного места (`loop.py`, один раз на BOOTSTRAP, не параллельно),
  гонка невоспроизводима по графу вызовов.** №38 (чистка завершённых записей
  автодожима) — попытался, поймал сам себя на том, что удаление записи после
  финального шага сделало бы `state.get(id,-1)==-1` неотличимым от «ещё не
  начинали» и ЗАПУСТИЛО БЫ серию заново — откатил, оставил как есть с
  объяснением в коде (незначительный рост на один int на лид — не стоит
  риска регресса). №34 (TeamView «фейковая» загрузка) — при проверке текст
  УЖЕ был честным («Загрузка…», не «нет ролей»), реальной пользы от правки
  не нашлось. №13/14/15/16 (крупные структурные рефакторы) — сознательно не
  трогались, см. пометку в самом чек-листе.

**Живая проверка на реальном API** (не только юнит-тесты): засеял тенант с
командой + реальным открытым вопросом дизайнера, реальный BOOTSTRAP успел
сгенерировать дашборд до постановки на паузу — подтвердил ещё раз gate из
§3.8 (`blocking:true` → recovery-экран → confirm), и по-настоящему подтвердил
новый баннер «Команда ждёт вашего ответа»: клик по нему в реальном браузере
корректно открыл личный чат дизайнера с текстом вопроса. Потрачено ~$0.003
на это подтверждение, тенант удалён.

Новые тесты: `test_costs_reservation.py` (6), `test_connections_ambiguous.py`
(5), `test_capability_gate.py` (4), `test_digest.py` (4), плюс расширены
`test_execution.py` (+1 вотчдог), `test_autonomy.py` (+3 declared action_type),
`test_lead_nurture.py` (+1 reopen-reset) — итого 73 тестовых файла (было 69),
все проходят. py_compile + `npx tsc --noEmit` + `tests/run_all.py` — чисто.

Дальше по инструкции: UX-копирайтинг с подсказками «для неопытного
пользователя» по всем вкладкам, и системный обзор экранов на предмет
нарушений логики / возможности переложить больше на ИИ.

---

## 2026-07-20 (продолжение 12) — Живой аудит реального цикла + фикс всех 8 находок

По инструкции «запусти реальный цикл и проверяй всё... но не исправляй
ничего, только запоминай» — прогнал полный реальный BOOTSTRAP (office-real,
настоящий API, $0.0144) для кофейни с доставкой, зафиксировал находки в
`docs/live-audit-findings-2026-07-20.md` БЕЗ единой правки кода. Следующим
сообщением получил «исправь всё это по порядку» — исправил все 8, каждый —
реальный код-баг, найденный либо API-сверкой, либо чтением кода.

**1+6 (один и тот же баг). «Хранилище» показывает 0, и вообще ВСЕ вкладки
долго грузятся.** Первопричина: `routers/work.py::get_storage_usage()` —
`async def`-хендлер, СИНХРОННО вызывающий `storage_usage.summary()`, а тот
внутри делает `subprocess.run(docker ...)` с таймаутом до 15-20с (Windows).
Без `asyncio.to_thread` это блокирует ВЕСЬ event loop процесса — не только
этот запрос, а вообще ВСЕ параллельные запросы всех тенантов (SSE, другие
API) на время вызова. Объясняет и «Хранилище показывает 0» (я проверял до
того, как блокирующий вызов освобождался), и «все вкладки долго грузятся»
(вообще всё стоит, пока Docker отвечает). Фикс — `await
asyncio.to_thread(storage_usage.summary)`. ⚠️ Тот же паттерн (`subprocess.run`
без to_thread в async-хендлере) найден и в `tenant_apps.py` (вкладка
«Приложения») — НЕ тронут в этом заходе (нет прямого репорта проблемы
именно там), зафиксирован как вероятный follow-up.

**2. «Маркетолог × 2» в карточке проекта — не число агентов.** `roleCounts`
в `ProjectView.tsx` считает ЗАДАЧИ этой роли в плане, не нанятых людей
(MAX_PER_ROLE=1 — их всегда 1, инвариант не нарушен, нарушена только
подпись). Живой прогон подтвердил: реальная команда — 7 агентов, каждая
роль ровно один раз, при этом карточка писала «× 2» для трёх ролей.
Переименовано в «· N задачи», не «× N».

**3. Задачи «без этапа», хотя реально относятся к активному этапу.**
`plan.set_tasks()` подписывает `milestone_id` ОДИН РАЗ, в момент генерации,
по "предполагаемому следующему" этапу (`milestones.active_stage_id` —
индекс сразу после последнего done). Если офис ПОСЛЕ этого реально
переключает фокус на ДРУГОЙ этап того же проекта (`decision_engine`/
`planning_engine` зовут `mark_active` в обход "предполагаемого" — живой
прогон: сгенерировал под "Контент", а офис прыгнул сразу к "Сборка бота"),
уже сгенерированные задачи навсегда остаются подписаны пропущенным этапом.
Новая `plan.resync_pending_milestone(project_id, stage_id)`, вызывается из
`milestones.mark_active()` — переподписывает только `status=="pending"`
задачи (уже начатые/готовые сохраняют исторически точную метку).

**4. «Прогресс 33%» при «Задачи 0/6» без объяснения.** Это ДВЕ разные
метрики (Прогресс — по бизнес-этапам, Задачи — по текущему плану), которые
законно могут расходиться, но нигде не было сказано, что они разные. Добавил
`title`-тултип на каждую карточку метрики (`Card` получил новый проп
`title`).

**5. Задвоенный/склеенный текст в первом сообщении онбординга.** Вероятно
артефакт повторного набора в неочищенное поле (не баг для живого
пользователя), НО бэкенд принимал и НАВСЕГДА сохранял в историю чата что
угодно без единой проверки длины. `investigation.run_turn()` теперь режет
на `_MAX_MESSAGE_CHARS=2000` и обрезает пробелы; фронт получил `maxLength`
на оба поля ввода онбординга + недостающий `preventDefault()` на Enter во
втором поле (для консистентности с первым).

**7. Агенты не распределены по проектам, пока не обновишь страницу.**
Нашёл при чтении кода в процессе починки (не был в исходном списке из
8, но того же класса): SSE-событие `"hired"` никогда не несло `project_id` —
`OfficeProvider.upsertAgent` переносит `projectId` из ПРЕДЫДУЩЕЙ записи
агента, а у свежесозданного предыдущей записи нет. `planning_engine.
hire_and_run()` (project-scoped наём) теперь публикует `project_id` в
событии; фронт передаёт его в `upsertAgent`. `hire_initial`/`hire_leader`
(штабные роли, без проекта по конструкции) не тронуты — там "Штаб" честен.

**8. Новый проект стартует сам, хотя ни одна инициатива не принята.**
Не баг — `projects.ensure_active()` в BOOTSTRAP всегда безусловно
создаёт проект из цели брифа, независимо от 3 предложенных инициатив (те
— надстройка сверху). Но экран «Вот что понял офис» никак это не объяснял.
Добавлена одна поясняющая строка прямо над карточками инициатив.

Тесты: `test_milestone_task_link.py` +3 (resync), `test_investigation.py`
+2 (обрезка длины/пробелы) — 73/73 файлов, все проходят (то же число, что
и до этой итерации — только новые тесты в существующих файлах, не новые
файлы). py_compile + `npx tsc --noEmit` + `tests/run_all.py` — чисто.

## 2026-07-20 (продолжение 13) — Раунд 2 аудита (5 старых + 10 новых сценариев) + фикс всех 15 находок + портрет §11

Продолжение того же дня: сначала прогнал раунд 2 живого аудита (5 ранее
непроверенных пункта из раунда 1 + 10 новых уникальных сценариев,
`docs/functional-gaps-round2-2026-07-20.md`, $0.0612), затем «исправь всё
по порядку» — исправил все 15 находок плюс единственный точечный (не
архитектурный) пункт из сверки портрета, §11. Тесты: 73→77 файлов
(4 новых: `test_decision_engine_pause.py`, `test_prior_blockers.py`,
`test_llm_dedupe.py` расширен, `test_workspace_upload.py`), py_compile +
`npx tsc --noEmit` + `tests/run_all.py` — чисто, живая проверка почти
каждого пункта (office-real, реальный API).

**U1. CEO подтверждает отмену работы словами, но не отменяет её реально.**
Самая серьёзная находка раунда: живой прогон — «забудьте про Telegram-бота»
→ CEO ответил «убираю из плана», но `interpret_directive` возвращал только
`new_tasks`/`milestone_ops`, никогда не трогая уже АКТИВНЫЕ проекты; 3
Telegram-проекта остались `active`, воркеры продолжали по ним отчитываться
в том же чате. `decision_engine.py`: PlanDiff получил `pause_projects:
[project_id]` (`_apply` вызывает `projects.pause`); `orchestrator.
interpret_directive` теперь подмешивает список активных проектов с id;
`policies/ceo_directive.md` учит модель заполнять это поле при реальной
отмене направления, с примером из этого же бага.

**N1. Устаревшая вкладка может воскресить уже отклонённую инициативу.**
`reject_initiative` не публиковал SSE — вторая открытая вкладка не узнавала
об отклонении. Хуже: `initiatives.accept()` не проверял текущий статус
вообще — старая карточка «Принять» в устаревшей вкладке реально переводила
`rejected` обратно в `accepted` и спавнила проект. Добавлен `bus.publish
("initiative_resolved")` на accept/reject (тихий счётчик `state.
initiativesTick` в `OfficeProvider`, без шума в ленте) + `InitiativeAlreadyResolved`
как guard в `accept()`/`reject()`. Заодно нашёл и починил СОСЕДНИЙ баг:
`api.post`/`postJSON` на 4xx отбрасывают тело ответа — ветка
`initiative_blocked` в `DashboardView` никогда не срабатывала на практике;
`postJSONReadBody` получил поддержку заголовков, `api.postReadBody`/
`acceptInitiative`/`rejectInitiative` переведены на него.

**U2-побочная. Ответ модели иногда приходит задвоенным дословно.** Личный
чат вернул ответ агента, буквально повторённый два раза подряд без
разделителя — воспроизведено чистым `fetch()`, без единого нажатия
клавиши, что ЗАДНИМ ЧИСЛОМ опровергает вывод раунда 1 (там идентичный
симптом списали на артефакт повторного набора текста в браузере — это
было неверно). `llm.py`: новая `_dedupe_repeated_output()` — если вторая
половина ответа модели совпадает с первой (точно или с точностью до
пробела на стыке), обрезает повтор перед `return`.

**N9. Истёкшая сессия — молчаливая деградация до пустых списков.**
`getJSON`/`postJSON`/`postJSONReadBody`/`del` на 401 ничем не отличались
от сетевого сбоя; `Gate.tsx` проверял auth один раз при монтировании.
Добавлен `setAuthExpiredHandler` в `api.ts` — любой 401 сбрасывает
приложение на лендинг с баннером «Сессия истекла — войдите заново» и
авто-открывает форму входа. Попутно нашёл и починил СОСЕДНИЙ баг: `/api/me`
для уже авторизованного пользователя не отдавал `github_available/
google_available/dev_login` — открывшаяся форма входа была бы пустой
(«методы входа не настроены»), сам механизм повторного логина выглядел бы
сломанным. Живой прогон подтвердил оба фикса разом.

**N6. Перезагрузка посреди онбординга стирает видимую историю (не данные).**
`InvestigationChat` вызывал `load()` только `if (chatting)`, а `chatting`
вычисляется ИЗ `messages` — курица-яйцо: на чистом маунте `load()` никогда
не запускался. Бэкенд (`office_channel`) всегда помнил всё, следующая
отправка сама «чинила» экран — баг был чисто визуальный, но реальный.
Один безусловный `useEffect(() => { load() }, [])`. Живой прогон: после
полной перезагрузки страницы (смена URL, ремаунт Gate) весь диалог
(«хочу денег» → CEO → «кофейня» → ...) появился сразу, без единого нажатия.

**N2. Разблокировка задачи стирала ВСЮ память о причине провала.**
`plan.unblock()` обнулял и `blocked_reason`, и `last_feedback` — исполнитель
после вмешательства владельца не знал, почему уже 3 раза провалился, и
рисковал повторить тот же провал. Новое поле `prior_blockers` (append,
максимум 3) сохраняет причину при unblock; `planning_engine.py` подмешивает
его в контекст задачи отдельной строкой, когда `last_feedback` уже пуст.

**Раунд1 #3. Нет лимита длины `/api/ask` и `/api/chat`.** Тот же класс, что
уже чинили для `investigation.py` в раунде 1, но не для общего/личного чата
— а там раздутая строка уходит в `memory.remember()` (подмешивается в
промпт КАЖДОГО будущего агента навсегда). `_MAX_MESSAGE_CHARS=2000` в
`routers/comms.py`, `maxLength` на инпуте `RightPanel.tsx`.

**N4. Нельзя удалить проект — только поставить на паузу навсегда.**
`projects.py`: новая `cancel(pid, note)` — статус `cancelled` (честно «не
доведено», не выдаёт себя за `done`), освобождает слот параллельности
(`_promote_queued`). `POST /api/project/{id}/cancel`; кнопка «🚫 Отменить»
в `ProjectDetailScreen` рядом с «⏸ Пауза», с `window.confirm`; пилюля
«🚫 Отменён» (не зелёная «Закрыт») в списке и карточке. Живой прогон
подтвердил весь цикл, включая правильную группировку в историю.

**U4. Индикатор расхода в TopBar недостижим с клавиатуры.** `<div onClick>`
→ `<button>`. Живой прогон: элемент появился в дереве интерактивных
(раньше отсутствовал).

**N8. Открытый вопрос агента безусловно поглощает любое следующее
сообщение как ответ.** `/api/ask` не различает «это ответ» и «это новая
команда» — полноценная классификация требует либо LLM-суждения, либо
явного UI-переключателя; сделан дешёвый, но настоящий минимум — баннер над
полем ввода в `ChatsView.tsx` показывает ТЕКСТ вопроса, на который ответ
засчитается, прежде чем пользователь начнёт печатать.

**Раунд1 #4. Экспорт результатов — раньше только полный отладочный лог.**
`GET /api/leads/export.csv` (с BOM для Excel) и `GET /api/project/{id}
/export.zip` (архив папки проекта в памяти). Кнопки в `ResultsView.tsx`
(лиды) и `ProjectDetailScreen` (проект). Живой прогон: оба скачивания
подтверждены прямым запросом (CSV — корректный заголовок, ZIP — 18KB,
`application/zip`).

**U3. Дайджест — одноразовый попап, теряется навсегда при случайном
закрытии.** `GET /api/digest` необратимо продвигает `last_seen` в момент
вызова, а `useEffect` на маунте зовёт его на КАЖДОЙ перезагрузке — если
попап закрылся/страница перезагрузилась до прочтения, контент утерян.
`digest.py`: `get_and_mark_seen()` дополнительно сохраняет последний
непустой дайджест в `last_digest.json`; новая `peek_last()` /
`GET /api/digest/last` не трогает `last_seen`. Кнопка «🌤» в TopBar
(видна, только если есть что переоткрыть) вызывает `reopenDigest()`.
Живой прогон: закрыл попап, нажал «🌤», содержимое вернулось дословно то же.

**Раунд1 #1. На мобильном нет способа добраться до чата откуда угодно.**
Самая серьёзная живая переоценка раунда: изначальный P0 («вообще нет UI
для ответа на блокирующий вопрос с телефона») оказался НЕ совсем точным —
путь Команда → карточка агента → «написать агенту» существовал и работал
(`ChatsView.tsx` уже был мобильно-адаптивным, свой брейкпоинт на 720px).
Реальная проблема — недостижимость С ЛЮБОГО экрана и нулевая
обнаруживаемость (счётчик непрочитанных нигде не виден вне «Команды»).
`NavRail.tsx`: `MOBILE_NAV` (только для `orientation="horizontal"`, десктоп
не тронут — оставлено намеренное решение «Чаты не отдельный пункт» из
рефакторинга 2026-07-05) вставляет «Чаты» с бейджем между «Командой» и
«Результатами». Живой прогон на 375×812: бейдж «Чаты, непрочитанных: 3»
виден с «Обзора», один тап → реальное поле «Написать CEO…» доступно.

**Раунд1 #2/2b. Нет загрузки файлов и vision — самая крупная находка.**
Ни multipart-эндпоинта, ни UI, ни единой функции в `core/llm.py`, умеющей
передать модели картинку, хотя `models.py` уже подписывает модели словом
«vision» в Настройках. `workspace.py`: новая `write_bytes()` (бинарные
данные, лимит 10MB, та же защита пути, что у `write_file`). `routers/
comms.py`: `POST /api/thread/{agent_id}/upload` (multipart, whitelist
image/audio/pdf) → `workspace.write_bytes` в `uploads/`. `core/llm.py`:
`run_agent` получил `image_urls` — `_build_user_message()` (вынесена
отдельно ради юнит-теста без реального API-клиента) собирает
OpenAI-формат `content: [text, image_url, ...]` вместо простой строки;
`chat.ask()` прокидывает дальше. `ChatsView.tsx`: кнопка 📎, скрытый
`<input type=file>`, чип вложения, для картинок — `FileReader.
readAsDataURL` на клиенте передаётся как `image_urls` в тот же `/api/ask`.
Живой прогон — САМОЕ убедительное доказательство сессии: сгенерировал
canvas-картинку со случайной строкой «BANAN42» (непроизносимое, негадаемое),
отправил `qwen3-vl-flash`-агенту через весь новый пайплайн — модель
ответила ровно «BANAN42». Загрузка через реальный `fetch(FormData)`
отдельно подтверждена (файл лежит в `workspace/uploads/`, читается через
уже существующий `/api/raw/`).

**Портрет §11. Мониторинг конкурентов не заводился по умолчанию.**
Единственный пункт сверки портрета (`docs/portrait-vs-code-2026-07-20.md`),
который был точечным кодовым багом, а не архитектурной темой на будущее —
портрет прямо требует «не по запросу, а по умолчанию для КАЖДОГО тенанта».
`loop.py` BOOTSTRAP: `processes.create("Мониторинг конкурентов", "marketer",
...)` сразу после `projects.ensure_active()` — сам `create()` уже
идемпотентен (дедуп похожего активного процесса того же проекта,
подтверждено существующим `test_processes.py`), поэтому безопасно звать
на каждом цикле BOOTSTRAP, реально создаётся один раз.

Осознанно НЕ взято в этот заход: N3 (бюджетный гейт — оценка ДО задачи, не
живой лимит внутри многошагового tool-loop) — архитектурно глубокая правка
низкой отдачи относительно риска, задокументирована как есть в раунде 2.
