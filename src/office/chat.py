...
# Чат-персоны ТОЛЬКО для штабных ролей CEO, у которых нет файла builtin_roles/*.md
# (их операционный текст живёт в собственных агент-модулях: researcher.py,
# strategist.py, architect.py). Отдельческие роли (developer/designer/marketer/…)
# СВОЮ личность берут из единого источника roles.render() — здесь их дублировать
# нельзя (это была расходящаяся вторая копия ролей, инцидент из аудита).
_SERVICE_PERSONA = {
    "orchestrator": (
        "Ты — директор AI-офиса. Ты управляешь всей командой, разбиваешь путь "
    ...
    "hr": (
        "Ты — HR-директор AI-офиса. Ты решаешь кого нанять, управляешь командой. "
...
def _role_persona(role: str) -> str:
    """Личность роли для чата из ЕДИНОГО источника: отдельческие роли — roles.render()
    (builtin_roles/*.md), штабные роли CEO — _SERVICE_PERSONA. Так личность агента в
    чате не расходится с тем, как он описан в реальной работе."""
    if role in roles_module.ROLE_META:
        return roles_module.render(role)
    return _SERVICE_PERSONA.get(
        role, f"Ты — {role} агент AI-офиса. Отвечай как дружелюбный коллега.")
