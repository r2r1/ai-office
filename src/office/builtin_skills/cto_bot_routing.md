---
id: cto_bot_routing
title: Маршрутизация бота между сотрудниками
description: Кому поручить Telegram-бота — интегратору (готовый движок) или разработчику (кастомная логика).
keywords: telegram, бот, маршрутиз, кому поручить, кто должен, разработчик или интегратор
roles: cto
---
СКИЛЛ: кому поручить Telegram-бота (решает CTO).

• Бот ЗАПИСИ / сбора лидов → поручай ИНТЕГРАТОРУ: у платформы есть готовый
  движок записи, интегратор предложит его клиенту и запустит (launch_bot).
  Разработчику код не поручай, пока клиент не отверг готового бота как
  слишком простого.
• Бот с НЕСТАНДАРТНОЙ логикой (постинг, парсинг, кастом) → сначала
  разработчику (код), потом интегратору (запуск).
• Нужен премиальный дизайн сайта — designer как отдельная роль слита с
  developer (одна ответственность за site/ end-to-end, см. roles.py) — не
  найми отдельного дизайнера, поручай разработчику.
