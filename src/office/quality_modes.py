"""
Режимы качества моделей (Quality Modes) — Execution Engine.

⚠️ Имя: НЕ путать с BOS Capability (§1/§5 — «умеем ли мы производить артефакт
такого типа»; это отдельная сущность, появится в Phase 2 как office/capability.py).
Здесь — только режимы качества выбора МОДЕЛИ под тип задачи. Модуль назывался
capabilities.py; переименован, чтобы не сталкиваться с термином BOS.

Пользователь мыслит не моделями, а качеством: выбирает РЕЖИМ (🟣 Экономия →
⚫ Эксперт), а система сама подбирает лучшую модель под ТИП задачи (capability-тип).
«Основная модель ≠ единственная модель»: код может уходить на одну модель,
картинки — на другую, видео — на третью.

Хранилище: data/tenants/<tid>/capabilities.json (имя файла оставлено прежним,
чтобы не терять уже сохранённые настройки тенантов при переименовании модуля).
