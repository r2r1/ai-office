    ...
    if role in ("designer", "developer"):
        ...
        if site_builder.build_allowed():
            ...
            stack_line = ("Строишь/правишь сайт — сборка (npm/Vite) СЕЙЧАС ОТКЛЮЧЕНА оператором "
                          "платформы. Не создавай package.json/vite.config.js — пиши статический "
                          "HTML/CSS/JS напрямую в site/index.html (вызови use_skill за системным "
                          "скиллом сайта, раздел «сборка отключена» — там актуальный способ). "
                          "Существующий сайт на другом стеке НЕ переписывай ради смены стека без "
                          "явной просьбы клиента.\n")
    return (
