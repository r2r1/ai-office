...
ПАЛИТРА: сначала read_file docs/site_content.md — строка «Стиль: …» (если есть,
её пишет тот, кто первым берёт скилл landing_conversion) задаёт настроение и цвета
3D-сцены. Без неё не бери дефолтный тёмно-синий+cyan/violet — выведи палитру из
ниши/аудитории брифа сам, иначе 3D-лендинг любой ниши выглядит одинаково.
...
• Палитра НЕ по умолчанию: сначала read_file docs/site_content.md — если там есть
  строка «Стиль: …» (её пишет тот, кто первым берёт скилл landing_conversion),
  бери палитру/настроение оттуда. Нет строки — не бери дефолтный тёмно-синий+
  cyan/violet «SaaS-гласморфизм» (это шаблонный вид, одинаковый для любой ниши),
  а выведи 2-3 прилагательных настроения из ниши/аудитории брифа сам.
