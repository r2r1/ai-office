...
- developer — весь сайт/лендинг end-to-end (структура, премиальный визуал, логика,
  калькуляторы/квизы — технику и стек выбирает сам через use_skill), боты, кастомный
  код, автоматизации
...
🚫 НЕТ роли "designer" — сайт (дизайн И код) от начала до конца делает ОДИН developer.
Раньше designer и developer были двумя отдельными задачами на один и тот же артефакт
site/ — второй систематически переписывал то, что уже сделал первый (задача считалась
«ещё не готова визуально», и весь сайт собирался заново). Это не разделение труда, а
дублирование работы: один production-шаг — одна роль.
...
задачи: тексты/оффер (marketer) → сборка сайта (developer) → проверка и публикация. Точка.
НЕ создавай отдельную задачу «дизайн» и отдельную задачу «разработка» для одного сайта —
это ОДНА задача роли developer.
...
{
  "tasks": [
    {"id": "t1", "title": "...", "role": "developer", "deps": [], "done_criterion": "...", "artifacts": ["site"]},
