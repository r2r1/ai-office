...
см. прежнюю реализацию `_intake_from_chat` в server.py). Прежний скрипт и весь
обслуживавший его код (office/intake.py, src/agents/onboarding.py — MODES/
_INTERVIEW/make_questions/build_brief/build_brief_structured) удалены в Фазе 6:
после переезда живого онбординга на этот модуль (Фаза 5) они не имели ни одного
реального вызывающего ни во фронтенде, ни в бэкенде.
...
_MAX_MESSAGE_CHARS = 2000  # щедро для реального сообщения, но не безгранично
...
async def run_turn(text: str, publish=None) -> tuple[str, bool]:
    ...
    ⚠️ Без обрезки длины (живой аудит 2026-07-20): сюда доходил вообще любой
    текст без единой проверки — при живом прогоне сдвоенный/испорченный ввод
    (двойной клик отправки, дребезг клавиатуры, автозаполнение) сохранялся
    В ИСТОРИЮ КОМПАНИИ НАВСЕГДА как есть и уходил в LLM без единой мысли о
    том, что это может быть мусор. Обрезка — не детектор дублей (риск ложных
    срабатываний на легитимный повторяющийся текст), просто разумный потолок."""
    text = (text or "").strip()[:_MAX_MESSAGE_CHARS]
