...
async def _run_office(tid: str) -> None:
    ...
    if not tech_design:
        # ⏸ Первый дашборд (аналитика/точки роста/инициативы выше) ждёт
        # подтверждения владельца, ПРЕЖДЕ чем офис впервые спроектирует ТЗ и
        # план на его основе (портрет §23) — самый нагруженный последствиями
        # вывод офиса о компании: если понимание неверно, продолжать строить
        # дальше — тратить циклы и доверие впустую. ⚠️ Обратная совместимость:
        # эта проверка стоит ВНУТРИ `if not tech_design`, который и раньше
        # пропускался для тенантов с уже спроектированным ТЗ — тенанты,
        # прошедшие BOOTSTRAP ДО появления этого гейта, сюда не попадают и
        # подтверждения задним числом не требуют. `has_content()` не даёт
        # застрять на пустом дашборде (генерация могла упасть).
        if onboarding_result.has_content() and not onboarding_result.is_confirmed():
            await publish({"type": "system",
                           "text": "⏸ Жду подтверждения дашборда — посмотрите анализ и нажмите "
                                   "«Продолжить», прежде чем офис спроектирует ТЗ и план."})
            while onboarding_result.has_content() and not onboarding_result.is_confirmed():
