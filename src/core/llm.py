...
async def run_agent(
    ...
    in_tokens = 0
    out_tokens = 0
    # Промпт-кеширование (docs/audit-dd-2026-07-06.md §8/§10): system (роль+политики
    # из prompt_builder.build — стабилен между вызовами ОДНОЙ роли) намеренно
    # держится ОТДЕЛЬНО от user (task_context — меняется на каждый вызов), а не
    # склеивается в один блок. Это уже даёт автоматическое кеширование префикса
    # на провайдерах, которые его поддерживают (OpenAI/Anthropic-совместимые API
    # кешируют повторяющийся system-префикс без явных маркеров). Явный
    # `cache_control` (Anthropic Messages API) сюда НЕ добавлен намеренно: apinet
    # проксирует разные модели одним OpenAI-совместимым интерфейсом, и неверно
    # угаданный формат маркера для модели, которая его не поддерживает, рискует
    # уронить вызов вместо экономии — нужна предварительная проверка по
    # документации конкретного апстрима apinet, а не слепая правка.
