...
class Integration:
    ...
    department: str = ""
    # Манифест провайдера (canonical spec §5.3/Part 14, продукт-портрет §24):
    # какие Facts (office/knowledge.py, знание с провенансом source="scanned")
    # способность УМЕЕТ поставлять в World Model, если её READ-действие
    # (list_events/read_email/read_sheet и т.п.) прогнать через knowledge.remember.
    # Короткие человекочитаемые метки, не фиксированный enum — knowledge.py
    # фактов сам не типизирует. ⚠️ Это ДЕКЛАРАЦИЯ намерения, не автоматическое
    # исполнение: сама запись в knowledge.py — отдельная работа по каждому
    # провайдеру (пока не сделана ни для одного — см. 🟡 в критическом аудите
    # спеки, "интеграции не декларируют добываемые факты"). Провайдеры без
    # read-действия, дающего содержательные данные (export-only вроде crm.py/
    # crm_bitrix24.py/erp_1c.py — только пишут лид наружу, не читают статус
    # обратно) намеренно оставлены с пустым списком — декларировать факт,
    # который сегодня физически неоткуда взять, было бы враньём каталога.
    produces_facts: list[str] = field(default_factory=list)
    # Цена ДО подключения — первоклассный элемент карточки, не мелкий шрифт
    # (продукт-портрет §24: «офис честен про цену», согласовано с §15). Пусто —
    # способность бесплатна (сегодня это ВСЕ интеграции каталога — платных
    # провайдеров ещё нет ни одного, дефолт честный, не заглушка на будущее).
    # Не структурированное число — большинство провайдеров либо бесплатны,
    # либо тарифицируются самим внешним сервисом (Bitrix24/1С), а не платформой;
    # свободный текст ("от $9/мес у провайдера" и т.п.) точнее одного числа.
    price_hint: str = ""
