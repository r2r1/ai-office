...
🚫 ВАЖНО: НЕ закладывай в стек платные конструкторы (Tilda, Webflow, Wix) и внешние CRM
(amoCRM, Bitrix24) — без явного одобрения клиента они под запретом. Сайт/лендинг — это
СОБСТВЕННЫЙ код (HTML5 + CSS3 + Vanilla JS), который агент пишет сам и публикует через
встроенную интеграцию `website` (она хостит лендинг и собирает лиды из коробки). Заявки
складывай в собственное хранилище (SQLite / Google Sheets API), а не в платную CRM.
