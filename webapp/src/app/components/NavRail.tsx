...
// "Лиды" → "Результаты" (product-manager разбор): лиды на одном уровне с
// "Работа"/"Команда" читались как ещё один процесс, хотя это ОДИН из
// результатов, которые работа команды производит — вкладка теперь показывает
// реестр типов результата (results.py: Лиды+Сайты сейчас, дальше — что
// зарегистрируется), с персональным порядком/видимостью под-вкладок.
const NAV: Array<{ id: Section; label: string; group: "top" | "bottom" }> = [
  ...
  { id: "results",     label: "Результаты", group: "top" },
